...
import mr_clean.functions.basics as _basics
...
def dtypes_summary(df):
    ...
    for colname in df:
        ...
        rows_numerical = _pd.to_numeric(data,errors = 'coerce').count()
        ...
        rows_date_time = _pd.to_datetime(data,errors = 'coerce',infer_datetime_format = True).count()
        ...
        # add to dataframe
        output_df.loc[:,colname] = _pd.Series(output_data)
...
def df_outliers(df):
    ...
    for col_name in df.columns:
        outlier_list[col_name] = outliers(df[col_name],'bool',dtypes[col_name])
...
def outliers(df,output_type = 'values',dtype = 'number'):# can output boolean array or values
    ...
    if dtype in ('number','datetime','timedelt','datetimetz'):
        if not dtype == 'number':
            df = _pd.to_numeric(df,errors = 'coerce')
        ...
        out_values = 1.5 * (quart75 - quart25) + quart75 + 1
        if out_values >= len(value_counts):
            bool_array = _utils.bc_vec(len(df),value = False)
        else:
            outlier_values = value_counts[value_counts <= value_counts[out_values]].index
