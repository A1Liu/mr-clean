...
def summarize(df,preview_rows = 2,
    ...
    # Reformat displays
    _pd.set_option('display.max_columns', display_max_cols)
    ...
    df_info = _io.get_info(df,verbose = True, max_cols = None, 
                               memory_usage = memory_usage,null_counts = True)
    
    percent_values = stats.percentiles(df)
    percent_values = 'Percentile Details skipped: No numerical data.' \
        if percent_values is None else percent_values
    ...
    output = ''
    for title, value in zip(title_list,info_list):
    ...
    if output_path is None:
         print(output)
    else: #TODO Add the directory support here.
